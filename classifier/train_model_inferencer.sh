...
mallet train-topics --input "${d}"/classifier/results/"${c}".mallet --output-state "${d}"/classifier/state/"${c}"-state.gz --output-doc-topics "${d}"/classifier/topics/"${c}"-topics.txt  --output-topic-keys "${d}"/classifier/topics/"${c}"-keys.txt --output-topic-docs "${d}"/classifier/topics/"${c}"-top.txt  --optimize-interval 10 --num-topics 18 --output-model "${d}"/classifier/results/"${c}"-model 
