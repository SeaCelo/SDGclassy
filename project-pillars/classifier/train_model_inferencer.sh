...
#Train topic model
mallet train-topics --input "${d}"/classifier/output/"${c}".mallet --output-state "${d}"/classifier/state/"${c}"-state.gz --output-doc-topics "${d}"/classifier/topics/"${c}"-topics.txt  --output-topic-keys "${d}"/classifier/topics/"${c}"-keys.txt --num-top-words 30 --optimize-interval 10 --num-topics 8 --output-model "${d}"/classifier/output/"${c}"-model 
